...
class ValueToLineMap : public AssemblyAnnotationWriter {
 ...
 public:
  ...
  // is printed.
  void printInfoComment(const Value &V, formatted_raw_ostream &Out) override { addEntry(&V, Out); }

  void emitBasicBlockStartAnnot(const BasicBlock *B, formatted_raw_ostream &Out) override {
    addEntry(B, Out);
  }

  void emitFunctionAnnot(const Function *F, formatted_raw_ostream &Out) override {
    addEntry(F, Out);
  }
...
class DIUpdater : public InstVisitor<DIUpdater> {
 ...
 public:
  DIUpdater(Module &M, StringRef Filename = StringRef(), StringRef Directory = StringRef(),
            const Module *DisplayM = nullptr, const ValueToValueMapTy *VMap = nullptr)
      : Builder(M),
        Layout(&M),
        LineTable(DisplayM ? DisplayM : &M),
        VMap(VMap),
        Finder(),
        Filename(Filename),
        Directory(Directory),
  ...
  void visitModule(Module &M) {
    ...
    if (Finder.compile_unit_count() > 1)
      report_fatal_error(
          "DebugIR pass supports only a signle compile unit per "
          "Module.");
  ...
  void visitFunction(Function &F) {
    if (F.isDeclaration() || findDISubprogram(&F)) return;
    ...
    if (!findLine(FirstInst, ScopeLine)) {
      LLVM_DEBUG(dbgs() << "WARNING: No line for 1st Instruction in Function " << F.getName().str()
                        << "\n");
      return;
    }
    ...
    if (IsOptimized) SPFlags |= DISubprogram::SPFlagOptimized;
    if (F.hasPrivateLinkage() || F.hasInternalLinkage()) SPFlags |= DISubprogram::SPFlagLocalToUnit;
    if (F.isDeclaration()) SPFlags |= DISubprogram::SPFlagDefinition;
    DISubprogram *Sub = Builder.createFunction(FileNode, F.getName(), MangledName, FileNode, Line,
                                               Sig, ScopeLine, DINode::FlagZero, SPFlags);
    F.setSubprogram(Sub);
    ...
    IRBuilder<> ArgIrBuilder(&F.getEntryBlock(), F.getEntryBlock().getFirstInsertionPt());
    for (size_t I = 0; I < F.arg_size(); I++) {
      auto *Arg = F.getArg(I);
      if (Arg->getName().empty()) continue;
      auto *Alloca = ArgIrBuilder.CreateAlloca(Arg->getType(), nullptr, Arg->getName());
      ArgIrBuilder.CreateStore(Arg, Alloca);

      // Scope must be the function for gdb to recognize this as a function
      // argument
      auto DILV = Builder.createParameterVariable(Sub, Arg->getName(), I + 1, FileNode, Line,
  ...
  void visitInstruction(Instruction &I) {
    ...
    if (VMap) RealInst = VMap->lookup(&I);

    if (!RealInst) RealInst = &I;
    ...
    if (!LineTable.getLine(RealInst, Line)) {
      ...
      LLVM_DEBUG(dbgs() << "WARNING: no LineTable entry for instruction " << RealInst << "\n");
      return;
    }
    ...
    DebugLoc NewLoc = DebugLoc(DILocation::get(M.getContext(), Line, Col, Scope, InlinedAt));
    ...
    if (!I.getType()->isVoidTy() && !I.getName().empty()) {
      ...
      Builder.insertDeclare(&I, DILV, Builder.createExpression(), NewLoc.get(), I.getParent());
    }
  }
 ...
 private:
  void createCompileUnit(DICompileUnit *CUToReplace) {
    ...
    if (CUToReplace) {
      ...
      Producer = "LLVM Version " STR(LLVM_VERSION_MAJOR) "." STR(LLVM_VERSION_MINOR);
    }

    FileNode = Builder.createFile(Filename, Directory);
    DICompileUnit *CU = Builder.createCompileUnit(dwarf::DW_LANG_C99, FileNode, Producer,
  ...
  DIScope *getBlockScope(DIScope *ParentScope, const BasicBlock *B) {
    ...
    if (BScope != BlockDescriptors.end()) {
      ...
      unsigned Line = 0;
      if (!findLine(B, Line)) {
        LLVM_DEBUG(dbgs() << "WARNING: No line for basic block " << B->getName().str()
                          << " in Function " << B->getParent()->getName().str() << "\n");
      }
  ...
  DIScope *findScope(const Instruction *I) {
    ...
    if (!SubprogramScope) return returnFallback();
  ...
  DISubprogram *findDISubprogram(const Function *F) {
    ...
    if (i != SubprogramDescriptors.end()) return i->second;

    LLVM_DEBUG(dbgs() << "searching for DI scope node for Function " << F << " in a list of "
                      << Finder.subprogram_count() << " subprogram nodes" << "\n");

    // TODO: When would this even be reached?
    for (DISubprogram *S : Finder.subprograms()) {
      // TODO: Is describes correct?
      if (S->describes(F)) {
        LLVM_DEBUG(dbgs() << "Found DISubprogram " << S << " for function " << F->getName()
                          << "\n");
        return S;
      }
    }
    LLVM_DEBUG(dbgs() << "unable to find DISubprogram node for function " << F->getName().str()
                      << "\n");
    return nullptr;
  }
  ...
  bool findLine(const Value *V, unsigned &Line) {
    if (LineTable.getLine(V, Line)) return true;

    if (VMap) {
      Value *mapped = VMap->lookup(V);
      if (mapped && LineTable.getLine(mapped, Line)) return true;
    }
    return false;
  }
  ...
  DIType *getType(const Type *T) {
    ...
    if (i != TypeDescriptors.end()) return i->second;
    return nullptr;
  }
  ...
  DIType *getOrCreateType(Type *T) {
    ...
    if (N)
      ...
      if (ST->isOpaque())
        ...
        SmallVector<Metadata *, 4> Elements;  // unfortunately, SmallVector<Type *> does not decay
                                              // to SmallVector<Metadata *>
        ...
        for (unsigned I = 0; I < T->getStructNumElements(); ++I) {
          ...
          DIType *MemType = Builder.createMemberType(
              ...
              (ST->hasName() ? T->getStructName().str() + "." + std::to_string(tempNameCounter++)
                             : "literal"),
              FileNode, 0, 0, 0, TLayout->getElementOffsetInBits(I), DINode::DIFlags::FlagZero,
              ElDIType);
      ...
      N = Builder.createPointerType(nullptr, Layout.getPointerTypeSizeInBits(T),
                                    Layout.getPrefTypeAlign(T).value() * CHAR_BIT,
                                    /*DWARFAddressSpace=*/std::nullopt, getTypeName(T));
    } else if (T->isArrayTy()) {
      SmallVector<Metadata *, 4> Subscripts;  // unfortunately, SmallVector<Type *> does not decay
                                              // to SmallVector<Metadata *>

      Subscripts.push_back(Builder.getOrCreateSubrange(0, T->getArrayNumElements() - 1));

      N = Builder.createArrayType(
          Layout.getTypeSizeInBits(T), Layout.getPrefTypeAlign(T).value() * CHAR_BIT,
          getOrCreateType(T->getArrayElementType()), Builder.getOrCreateArray(Subscripts));
    } else {
      ...
      N = Builder.createBasicType(getTypeName(T), T->getPrimitiveSizeInBits(), encoding);
    }
    TypeDescriptors[T] = N;
    return N;
  }
...
void createDebugInfo(Module &M, std::string Directory, std::string Filename) {
  StripDebugInfo(M);
  {
    ...
    DIUpdater R(M, Filename, Directory);
  }
